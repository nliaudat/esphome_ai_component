...
enum ImageProcessorInputType { kInputTypeFloat32, kInputTypeUInt8, kInputTypeUnknown };
...
struct ImageProcessorConfig {
  ...
  bool validate() const {
    return camera_width > 0 && camera_height > 0 && !pixel_format.empty() && model_width > 0 && model_height > 0 &&
           model_channels > 0;
  }
};
...
class ImageProcessor {
 public:
  ...
  std::vector<ProcessResult> split_image_in_zone(std::span<uint8_t> image_data, int width, int height,
                                                 const std::vector<CropZone> &zones);

  // Legacy wrapper
  std::vector<ProcessResult> split_image_in_zone(std::shared_ptr<camera::CameraImage> image,
                                                 const std::vector<CropZone> &zones);

  /**
   * Process a single zone.
   */
  ProcessResult process_zone(std::shared_ptr<camera::CameraImage> image, const CropZone &zone);

  /**
   * Process a single zone directly into a provided buffer.
   */
  bool process_zone_to_buffer(std::shared_ptr<camera::CameraImage> image, const CropZone &zone, uint8_t *output_buffer,
  ...
  void draw_filled_rectangle(uint8_t *buffer, int x, int y, int w, int h, int img_w, int img_h, int channels,
                             uint16_t color) {
 ...
 private:
  ...
  bool process_jpeg_zone_to_buffer(std::shared_ptr<camera::CameraImage> image, const CropZone &zone,
                                   uint8_t *output_buffer, size_t output_buffer_size);

  bool process_raw_zone_to_buffer(std::shared_ptr<camera::CameraImage> image, const CropZone &zone,
                                  uint8_t *output_buffer, size_t output_buffer_size);

  // Helper for raw pointer processing (no CameraImage dependency)
  bool process_raw_zone_pointer(const uint8_t *input_data, size_t input_len, int width, int height,
                                const CropZone &zone, uint8_t *output_buffer, size_t output_buffer_size);

  // Helper processing methods
  bool process_rgb888_crop_and_scale_to_float32(const uint8_t *input_data, const CropZone &zone, int crop_width,
                                                int crop_height, uint8_t *output_buffer, int model_width,
                                                int model_height, int channels, bool normalize, int src_stride_width);
  bool process_rgb888_crop_and_scale_to_uint8(const uint8_t *input_data, const CropZone &zone, int crop_width,
                                              int crop_height, uint8_t *output_buffer, int model_width,
                                              int model_height, int channels, int src_stride_width);

  bool process_rgb565_crop_and_scale_to_float32(const uint8_t *input_data, const CropZone &zone, int crop_width,
                                                int crop_height, uint8_t *output_buffer, int model_width,
                                                int model_height, int model_channels, bool normalize,
                                                int src_stride_width);
  bool process_rgb565_crop_and_scale_to_uint8(const uint8_t *input_data, const CropZone &zone, int crop_width,
                                              int crop_height, uint8_t *output_buffer, int model_width,
                                              int model_height, int model_channels, int src_stride_width);

  bool process_grayscale_crop_and_scale_to_float32(const uint8_t *input_data, const CropZone &zone, int crop_width,
                                                   int crop_height, uint8_t *output_buffer, int model_width,
                                                   int model_height, int model_channels, bool normalize,
                                                   int src_stride_width);
  bool process_grayscale_crop_and_scale_to_uint8(const uint8_t *input_data, const CropZone &zone, int crop_width,
                                                 int crop_height, uint8_t *output_buffer, int model_width,
                                                 int model_height, int model_channels, int src_stride_width);
 ...
 public:
   ...
   */
  static std::shared_ptr<camera::CameraImage> generate_rotated_preview(std::shared_ptr<camera::CameraImage> source,
  ...
  [[nodiscard]] static bool apply_software_rotation(const uint8_t *input, uint8_t *output, int src_w, int src_h,
  ...
  [[nodiscard]] static JpegBufferPtr decode_jpeg(const uint8_t *data, size_t len, int *width, int *height,
                                                 jpeg_pixel_format_t output_format = JPEG_PIXEL_FORMAT_RGB888,
                                                 jpeg_rotate_t rotate = JPEG_ROTATE_0D);
  ...
  void debug_analyze_float_zone(const float *data, int width, int height, int channels, const char *zone_name,
                                bool normalized);
  void debug_output_zone_preview(const uint8_t *data, int width, int height, int channels, const char *zone_name);
  void debug_output_float_preview(const float *data, int width, int height, int channels, const char *zone_name,
                                  bool normalized);
#endif
};
