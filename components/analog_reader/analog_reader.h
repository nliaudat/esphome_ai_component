...
class AnalogReader : public PollingComponent, public esphome::camera::CameraListener {
 ...
 protected:
  ...
  void preprocess_image(const uint8_t *img, int w, int h, int cx, int cy, int radius, NeedleType needle_type,
  ...
  void apply_tophat(uint8_t *img, int w, int h, int kernel_size, std::vector<uint8_t> &scratch,
