...
void ReadingHistory::cleanup_old_readings(uint32_t current_timestamp) {
  ...
  while (!hour_readings_.empty() && (current_timestamp - hour_readings_.front().timestamp) > HOUR_MS) {
    hour_readings_.pop_front();
  }

  while (!day_readings_.empty() && (current_timestamp - day_readings_.front().timestamp) > DAY_MS) {
...
bool OutputValidator::is_digit_plausible(int new_reading, int last_reading) const {
  ...
  if (absolute_diff > config_.max_absolute_diff) {
    ESP_LOGW(TAG, "Absolute difference too large: %d (max: %d)", absolute_diff, config_.max_absolute_diff);
    return false;
  }
  ...
  if (!config_.allow_negative_rates && new_reading < last_reading) {
    ...
    if (negative_diff > 5) {  // Allow very small negative changes (up to 5 units)
      ESP_LOGW(TAG, "Negative rate detected: %d -> %d (diff: %d)", last_reading, new_reading, negative_diff);
      return false;
    }
  }
...
int OutputValidator::apply_smart_validation(int new_reading, float confidence) {
  ...
  if (!is_digit_plausible(new_reading, last_valid_reading_)) {
    ...
    if (!recent_readings.empty()) {
      ...
      if (plausible_reading != new_reading && is_digit_plausible(plausible_reading, last_valid_reading_)) {
        ESP_LOGI(TAG, "Using plausible reading from history: %d (was: %d)", plausible_reading, new_reading);
