...
#define DEBUG_ZONE_INFO(zone, crop_w, crop_h, model_w, model_h) \
  ESP_LOGI(TAG, "ZONE: [%d,%d,%d,%d] -> %dx%d -> %dx%d", zone.x1, zone.y1, zone.x2, zone.y2, crop_w, crop_h, model_w, \
           model_h)
...
#define DEBUG_CHANNEL_ORDER(data, count, channels) \
  do { \
    if (channels >= 3) { \
      ...
      ESP_LOGI(TAG, "  First pixel: %.1f, %.1f, %.1f", data[0], data[1], data[2]); \
...
const char *ImageProcessor::jpeg_error_to_string(jpeg_error_t error) const {
  switch (error) {
    ...
    case JPEG_ERR_NO_MEM:
      return "Insufficient memory for the image";
    case JPEG_ERR_NO_MORE_DATA:
      return "Input data is not enough";
...
ImageProcessor::JpegBufferPtr ImageProcessor::decode_jpeg(const uint8_t *data, size_t len, int *width, int *height,
                                                          jpeg_pixel_format_t output_format, jpeg_rotate_t rotate) {
  if (!data || len == 0)
    return nullptr;
  ...
  if (out_size_64 > MAX_JPEG_OUTPUT || out_size_64 > SIZE_MAX) {
    ESP_LOGE(TAG, "JPEG output size too large: %llu bytes (dimensions: %dx%dx%d)", out_size_64, w, h, channels);
    return nullptr;
  }
...
std::shared_ptr<camera::CameraImage> ImageProcessor::generate_rotated_preview(
    std::shared_ptr<camera::CameraImage> source, float rotation, int width, int height) {
  if (!source)
    return nullptr;
  ...
  JpegBufferPtr full_image_buf =
      decode_jpeg(jpeg_data, jpeg_size, &dec_w, &dec_h, JPEG_PIXEL_FORMAT_RGB888, hw_rotation);
  if (!full_image_buf) {
  ...
  if (software_rotation_needed && std::abs(rotation) > 0.01f) {
    ...
    if (rot_buf) {
      bool rot_success =
          Rotator::perform_rotation(full_image_buf.get(), rot_buf.get(), dec_w, dec_h, 3, rotation, new_w, new_h);
  ...
  return std::shared_ptr<RotatedPreviewImage>(
      new RotatedPreviewImage(std::move(out_buffer), out_size, dec_w, dec_h, PIXFORMAT_RGB888));
}
#endif

bool ImageProcessor::validate_buffer_size(size_t required, size_t available, const char *context) const {
  if (available < required) {
    ESP_LOGE(TAG, "Buffer too small for %s: need %zu bytes, have %zu bytes", context, required, available);
    return false;
  }
  return true;
}
...
ImageProcessor::ImageProcessor(const ImageProcessorConfig &config) : config_(config) {
  if (!this->config_.validate()) {
    ...
    ESP_LOGE(TAG, "  Camera: %dx%d, Format: %s", this->config_.camera_width, this->config_.camera_height,
...
size_t ImageProcessor::get_required_buffer_size() const {
  if (this->config_.input_type == kInputTypeFloat32) {
    uint64_t size = static_cast<uint64_t>(this->config_.model_width) * this->config_.model_height *
                    this->config_.model_channels * sizeof(float);
    if (size > SIZE_MAX)
      return 0;
    return static_cast<size_t>(size);
  } else if (this->config_.input_type == kInputTypeUInt8) {
    uint64_t size =
        static_cast<uint64_t>(this->config_.model_width) * this->config_.model_height * this->config_.model_channels;
    if (size > SIZE_MAX)
      return 0;
    return static_cast<size_t>(size);
  }
...
std::vector<ImageProcessor::ProcessResult> ImageProcessor::split_image_in_zone(
    std::shared_ptr<camera::CameraImage> image, const std::vector<CropZone> &zones) {
...
std::vector<ImageProcessor::ProcessResult> ImageProcessor::split_image_in_zone(std::span<uint8_t> image_data, int width,
                                                                               int height,
  ...
  // Only applies if format is JPEG (ProcessRaw is already efficient-ish, though strict raw doesn't allocate
  ...
  ESP_LOGD(TAG, "Image Processing Start. PSRAM Free: %zu KB (Threshold: %zu KB). Format: %s", free_spiram / 1024,
           SPIRAM_RESERVE_THRESHOLD / 1024, this->config_.pixel_format.c_str());
  ...
  if (!is_actually_jpeg && image_data.size() >= 2 && image_data[0] == 0xFF && image_data[1] == 0xD8) {
...
// 3. Rotate Master if needed (Software fallback for fine angles)
    ...
    if (software_rotation_needed && std::abs(this->config_.rotation) > 0.01f) {
      ...
      if (raw_rot) {
        bool rot_success = Rotator::perform_rotation(master_decoded_buffer.get(), raw_rot, master_width, master_height,
                                                     master_channels, this->config_.rotation, rot_w, rot_h);
  ...
  if (use_master_buffer && master_decoded_buffer) {
    ...
    if (master_channels == 3 && this->config_.model_channels == 1) {
      ...
      if (gray_master_buffer) {
        ...
        master_channels = 1;
        ESP_LOGD(TAG, "Converted master buffer to grayscale: %dx%d (%zu bytes)", master_width, master_height,
                 gray_size);
      } else {
    ...
    if (this->config_.cache_preview_image) {
      ...
      if (preview_buf) {
        ...
        std::shared_ptr<RotatedPreviewImage> cached_img(
            new RotatedPreviewImage(std::move(preview_buf), buf_len, master_width, master_height,
  ...
  for (const auto &zone : effective_zones) {
    ...
    if (!out_buf) {
      ESP_LOGE(TAG, "Failed to allocate buffer of size %zu for zone [%d,%d,%d,%d]", required_size, zone.x1, zone.y1,
               zone.x2, zone.y2);
    ...
    if (use_master_buffer) {
      ...
      if (zone.x1 >= 0 && zone.y1 >= 0 && zone.x2 <= master_width && zone.y2 <= master_height) {
        ...
        if (master_channels == 1) {
          ...
          if (this->config_.input_type == kInputTypeFloat32) {
            crop_success = this->process_grayscale_crop_and_scale_to_float32(
                processing_source_ptr, zone, crop_w, crop_h, out_buf->get(), this->config_.model_width,
                this->config_.model_height, this->config_.model_channels, this->config_.normalize, stride);
          } else {
            crop_success = this->process_grayscale_crop_and_scale_to_uint8(
                processing_source_ptr, zone, crop_w, crop_h, out_buf->get(), this->config_.model_width,
                this->config_.model_height, this->config_.model_channels, stride);
            if (!crop_success) {
              ESP_LOGE(TAG, "process_grayscale... failed for zone [%d,%d,%d,%d]", zone.x1, zone.y1, zone.x2, zone.y2);
            }
          }
        } else {
          // RGB
        ...
        ESP_LOGE(TAG, "Zone bounds error vs Master: [%d,%d->%d,%d] in %dx%d", zone.x1, zone.y1, zone.x2, zone.y2,
                 master_width, master_height);
      }
    } else {
      // Legacy/Raw CameraImage path - adapted for span
      // Implementation of raw zone extraction using the span data:
      if (this->process_raw_zone_pointer(image_data.data(), image_data.size(), width, height, zone, out_buf->get(),
                                         required_size)) {
        crop_success = true;
      } else {
...
#if defined(DEBUG_ESP32_CAMERA_UTILS) || defined(DEBUG_METER_READER_TFLITE) || \
    defined(DEBUG_OUT_PROCESSED_IMAGE_TO_SERIAL)
      DEBUG_ZONE_INFO(zone, (zone.x2 - zone.x1), (zone.y2 - zone.y1), this->config_.model_width,
                      this->config_.model_height);
      // Full pixel analysis for analyze_serial_output.py (matches ZONE_ANALYSIS format)
      {
        ...
        ESP_LOGI(TAG, "ZONE_ANALYSIS:zone%zu:%dx%dx%d:normalized=false", results.size() + 1, w, h, c);
        ...
        if (pixel_data && w > 0 && h > 0) {
          ...
          if (count > 0) {
            ESP_LOGI(TAG, "Stats: min=%.3f, max=%.3f, mean=%.3f", min_val, max_val, static_cast<float>(sum) / count);
          }
        }
      }
#endif
...
bool ImageProcessor::process_zone_to_buffer(std::shared_ptr<camera::CameraImage> image, const CropZone &zone,
  ...
  if (zone.x1 < 0 || zone.y1 < 0 || zone.x2 > max_w || zone.y2 > max_h) {
    ESP_LOGE(TAG, "Zone out of bounds: [%d,%d->%d,%d] for image %dx%d (rot %d)", zone.x1, zone.y1, zone.x2, zone.y2,
             max_w, max_h, static_cast<int>(this->config_.rotation));
    return false;
  }
...
bool ImageProcessor::process_jpeg_zone_to_buffer(std::shared_ptr<camera::CameraImage> image, const CropZone &zone,
  ...
  if (get_jpeg_dimensions(jpeg_data, jpeg_size, jpeg_width, jpeg_height)) {
    if (jpeg_width != this->config_.camera_width || jpeg_height != this->config_.camera_height) {
      ...
      ESP_LOGD(TAG, "JPEG dimensions (%dx%d) differ from config (%dx%d). Using actual dimensions.", jpeg_width,
               jpeg_height, this->config_.camera_width, this->config_.camera_height);
    }
  } else {
  ...
  if (zone.x1 < 0 || zone.y1 < 0 || zone.x2 > jpeg_width || zone.y2 > jpeg_height) {
    ESP_LOGE(TAG, "Crop zone (%d,%d -> %d,%d) out of bounds for actual image (%dx%d) - Config: %dx%d", zone.x1, zone.y1,
             zone.x2, zone.y2, jpeg_width, jpeg_height, this->config_.camera_width, this->config_.camera_height);
    return false;
  }
  ...
  if (needs_grayscale_conversion && decode_channels == 3) {
    ...
    if (gray_buffer_storage) {
      ...
      ESP_LOGD(TAG, "Converted zone buffer to grayscale: %dx%d (%zu bytes)", jpeg_width, jpeg_height, gray_size);
    } else {
  ...
  if (software_rotation_needed && std::abs(this->config_.rotation) > 0.01f) {
    ...
    bool rot_success = Rotator::perform_rotation(processing_buf, rotated_buf, jpeg_width, jpeg_height, decode_channels,
                                                 this->config_.rotation, out_w, out_h);
    ...
    if (rot_success) {
      ...
      if (this->config_.cache_preview_image) {
        ...
        std::shared_ptr<RotatedPreviewImage> cached_img(
            new RotatedPreviewImage(std::move(rotated_buffer_storage), rotated_size, out_w, out_h,
  ...
  if (decode_channels == 1) {
    ...
    if (this->config_.input_type == kInputTypeFloat32) {
      scale_success = this->process_grayscale_crop_and_scale_to_float32(
          processing_buf, zone, crop_width, crop_height, output_buffer, this->config_.model_width,
          this->config_.model_height, this->config_.model_channels, this->config_.normalize, stride);
    } else if (this->config_.input_type == kInputTypeUInt8) {
...
ImageProcessor::ProcessResult ImageProcessor::process_zone(std::shared_ptr<camera::CameraImage> image,
                                                           const CropZone &zone) {
  ProcessResult result;

  if (!this->validate_input_image(image))
    return result;
  if (!this->validate_zone(zone))
    return result;
...
// Calculate effective max dimensions after rotation
  ...
  if (zone.x1 < 0 || zone.y1 < 0 || zone.x2 > max_w || zone.y2 > max_h) {
    ESP_LOGE(TAG, "Zone validation failed: [%d,%d->%d,%d] vs Max [%d,%d] (Rot: %.1f)", zone.x1, zone.y1, zone.x2,
             zone.y2, max_w, max_h, this->config_.rotation);
    return false;
  }
...
bool ImageProcessor::process_raw_zone_to_buffer(std::shared_ptr<camera::CameraImage> image, const CropZone &zone,
                                                uint8_t *output_buffer, size_t output_buffer_size) {
  if (!this->validate_input_image(image))
    return false;

  return this->process_raw_zone_pointer(image->get_data_buffer(), image->get_data_length(), this->config_.camera_width,
                                        this->config_.camera_height, zone, output_buffer, output_buffer_size);
}

bool ImageProcessor::process_raw_zone_pointer(const uint8_t *input_data, size_t input_len, int width, int height,
                                              const CropZone &zone, uint8_t *output_buffer, size_t output_buffer_size) {
  // Safety check: detect JPEG data that might have slipped through
  if (input_len >= 2 && input_data[0] == 0xFF && input_data[1] == 0xD8) {
    ESP_LOGE(TAG,
             "JPEG data detected in raw processing path! pixel_format='%s'. "
  ...
  if (zone.x1 < 0 || zone.y1 < 0 || zone.x2 > width || zone.y2 > height) {
    ESP_LOGE(TAG, "Zone out of bounds for raw processing: [%d,%d->%d,%d] in %dx%d", zone.x1, zone.y1, zone.x2, zone.y2,
             width, height);
    return false;
  }
  ...
  if (this->config_.pixel_format == "RGB888") {
    if (this->config_.input_type == kInputTypeFloat32) {
      ...
      success = this->process_rgb888_crop_and_scale_to_uint8(input_data, zone, crop_width, crop_height, target_buffer,
    ...
    if (this->config_.input_type == kInputTypeFloat32) {
      ...
      success = this->process_rgb565_crop_and_scale_to_uint8(input_data, zone, crop_width, crop_height, target_buffer,
                                                             scale_width, scale_height, this->config_.model_channels,
                                                             width);  // Use original width for stride
    }
  } else if (this->config_.pixel_format == "GRAYSCALE") {
...
// Apply rotation if needed
  ...
  if (success && needs_rotation) {
    ...
    success = this->apply_software_rotation(target_buffer, output_buffer, scale_width, scale_height, bytes_per_pixel,
                                            this->config_.rotation, rotated_w, rotated_h);
  }
#endif
...
bool ImageProcessor::scale_rgb888_to_float32(const uint8_t *src, int src_w, int src_h, uint8_t *dst, int dst_w,
                                             int dst_h, int channels, bool normalize) {
...
bool ImageProcessor::scale_rgb888_to_uint8(const uint8_t *src, int src_w, int src_h, uint8_t *dst, int dst_w, int dst_h,
                                           int channels) {
#ifdef USE_CAMERA_SCALER
  ...
  for (int y = 0; y < dst_h; y++) {
    for (int x = 0; x < dst_w; x++) {
      ...
      if (channels == 3) {
        ...
        dst[dst_idx] =
            static_cast<uint8_t>(0.299f * src[src_idx] + 0.587f * src[src_idx + 1] + 0.114f * src[src_idx + 2]);
      }
    }
  }
  return true;
...
bool ImageProcessor::process_rgb888_crop_and_scale_to_float32(const uint8_t *input_data, const CropZone &zone,
                                                              int crop_width, int crop_height, uint8_t *output_buffer,
                                                              int model_width, int model_height, int channels,
...
bool ImageProcessor::process_rgb888_crop_and_scale_to_uint8(const uint8_t *input_data, const CropZone &zone,
                                                            int crop_width, int crop_height, uint8_t *output_buffer,
                                                            int model_width, int model_height, int channels,
...
bool ImageProcessor::process_rgb565_crop_and_scale_to_float32(const uint8_t *input_data, const CropZone &zone,
                                                              int crop_width, int crop_height, uint8_t *output_buffer,
...
bool ImageProcessor::process_rgb565_crop_and_scale_to_uint8(const uint8_t *input_data, const CropZone &zone,
                                                            int crop_width, int crop_height, uint8_t *output_buffer,
                                                            int model_width, int model_height, int model_channels,
                                                            int src_stride_width) {
  const uint16_t *rgb565_data = reinterpret_cast<const uint16_t *>(input_data);
...
bool ImageProcessor::process_grayscale_crop_and_scale_to_float32(const uint8_t *input_data, const CropZone &zone,
                                                                 int crop_width, int crop_height,
                                                                 uint8_t *output_buffer, int model_width,
...
bool ImageProcessor::process_grayscale_crop_and_scale_to_uint8(const uint8_t *input_data, const CropZone &zone,
                                                               int crop_width, int crop_height, uint8_t *output_buffer,
                                                               int model_width, int model_height, int model_channels,
                                                               int src_stride_width) {
  float x_scale = static_cast<float>(crop_width) / model_width;
...
void ImageProcessor::arrange_channels(float *output, uint8_t r, uint8_t g, uint8_t b, int output_channels,
...
void ImageProcessor::arrange_channels(uint8_t *output, uint8_t r, uint8_t g, uint8_t b, int output_channels) const {
  if (output_channels >= 3) {
    if (this->config_.input_order == "BGR") {
      output[0] = b;
      output[1] = g;
...
#ifdef USE_CAMERA_ROTATOR
bool ImageProcessor::apply_software_rotation(const uint8_t *input, uint8_t *output, int width, int height,
                                             int bytes_per_pixel, float rotation_deg, int &out_w, int &out_h) {
#ifdef USE_CAMERA_ROTATOR
...
void ImageProcessor::debug_log_image_stats(const uint8_t *data, size_t size, const char *stage) {
  if (!data || size == 0)
    return;
  ...
  ESP_LOGD(TAG, "DEBUG %s: size=%zu, min=%u, max=%u, mean=%.1f, zeros=%d/%zu (%.1f%%)", stage, size, min_val, max_val,
           mean, zero_count, size, (zero_count * 100.0f) / size);
  ...
  for (int i = 0; i < std::min(10, static_cast<int>(size)); i++) {
    ...
    if (w < 0 || static_cast<size_t>(w) >= sizeof(buf) - offset)
      break;
    offset += w;
  }
  ESP_LOGD(TAG, "  %s", buf);
...
void ImageProcessor::debug_log_float_stats(const float *data, size_t count, const char *stage) {
  if (!data || count == 0)
    return;
  ...
  ESP_LOGD(TAG, "DEBUG %s: count=%zu, min=%.3f, max=%.3f, mean=%.3f", stage, count, min_val, max_val, mean);
...
void ImageProcessor::debug_log_image(const uint8_t *data, size_t size, int width, int height, int channels,
                                     const char *stage) {
  if (!data || size == 0)
    return;
  ESP_LOGD(TAG, "DEBUG %s: %dx%dx%d (%zu bytes)", stage, width, height, channels, size);
  debug_log_image_stats(data, size, stage);
}

void ImageProcessor::debug_log_float_image(const float *data, size_t count, int width, int height, int channels,
                                           const char *stage) {
  if (!data || count == 0)
    return;
  ESP_LOGD(TAG, "DEBUG %s: %dx%dx%d (%zu floats)", stage, width, height, channels, count);
  debug_log_float_stats(data, count, stage);
}

void ImageProcessor::debug_log_rgb888_image(const uint8_t *data, int width, int height, const char *stage) {
  debug_log_image(data, width * height * 3, width, height, 3, stage);
}

void ImageProcessor::debug_analyze_processed_zone(const uint8_t *data, int width, int height, int channels,
...
void ImageProcessor::debug_analyze_float_zone(const float *data, int width, int height, int channels,
                                              const char *zone_name, bool normalized) {
  if (!data || width <= 0 || height <= 0)
    return;
  debug_log_float_stats(data, width * height * channels, zone_name);
}

void ImageProcessor::debug_output_zone_preview(const uint8_t *data, int width, int height, int channels,
                                               const char *zone_name) {
  if (width > 64 || height > 64)
    return;  // Too big for log
...
void ImageProcessor::debug_output_float_preview(const float *data, int width, int height, int channels,
                                                const char *zone_name, bool normalized) {
  if (width > 64 || height > 64)
    return;
