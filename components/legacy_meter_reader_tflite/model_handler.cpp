...
bool ModelHandler::load_model(const uint8_t *model_data, size_t model_size, uint8_t *tensor_arena,
  ...
  interpreter_ = std::make_unique<tflite::MicroInterpreter>(tflite_model_, resolver, tensor_arena, tensor_arena_size);
  ...
  if (input->type == kTfLiteFloat32) {
    ...
    for (int i = 0; i < sample_size; i++) {
      ...
      if (config_.input_channels >= 3 && i % config_.input_channels == 2) {
        ESP_LOGD(TAG, "    -> %s: [%.3f, %.3f, %.3f]", config_.input_order.c_str(), data[i - 2], data[i - 1], data[i]);
      }
    }
  } else {
    ...
    for (int i = 0; i < sample_size; i++) {
      ...
      // Log channel groups for RGB/BGR
      if (config_.input_channels >= 3 && i % config_.input_channels == 2) {
        ESP_LOGD(TAG, "    -> %s: [%u, %u, %u]", config_.input_order.c_str(), data[i - 2], data[i - 1], data[i]);
      }
    }
  }
}
...
bool ModelHandler::invoke_model(const uint8_t *input_data, size_t input_size) {
  ...
  if (input_size != static_cast<size_t>(input->bytes)) {
    ...
    ESP_LOGE(TAG, "Model input dimensions: %dx%dx%d", get_input_width(), get_input_height(), get_input_channels());

    // FIXED: Use correct element size for calculation
    size_t element_size = (input->type == kTfLiteFloat32) ? sizeof(float) : sizeof(uint8_t);
    ESP_LOGE(TAG, "Expected elements: %zu, Actual elements: %zu", static_cast<size_t>(input->bytes) / element_size,
  ...
  if (input_size != expected_bytes) {
    ...
    ESP_LOGE(TAG, "Model shape: %dx%dx%d, type: %s", get_input_width(), get_input_height(), get_input_channels(),
             input->type == kTfLiteFloat32 ? "float32" : "uint8");
    return false;
  }

  // Check if input_data is valid
  ESP_LOGD(TAG, "Input data info: pointer=%p, size=%zu, tensor bytes=%d, type=%s", input_data, input_size, input->bytes,
           input->type == kTfLiteFloat32 ? "float32" : "uint8");
  ...
  if (input->type == kTfLiteUInt8) {
    ...
    ESP_LOGD(TAG, "Quantized input (uint8) - scale: %.6f, zero_point: %d", input->params.scale,
    ...
    ESP_LOGD(TAG, "Quantized input (int8) - scale: %.6f, zero_point: %d", input->params.scale,
             input->params.zero_point);
  ...
  if (output->type == kTfLiteUInt8 || output->type == kTfLiteInt8) {
    ...
    ESP_LOGD(TAG, "Quantized output (%s) - scale: %.6f, zero_point: %d",
             output->type == kTfLiteUInt8 ? "uint8" : "int8", scale, zero_point);
#endif
  ...
  ESP_LOGD(TAG, "Processed output - Value: %.1f, Confidence: %.6f", processed_output_.value,
           processed_output_.confidence);
...
bool ModelHandler::validate_model_config() {
  ...
  ESP_LOGI(TAG, "INPUT: type=%d, scale=%.6f, zp=%d", input->type, input->params.scale, input->params.zero_point);

  TfLiteTensor *output = output_tensor();
  if (output) {
    ESP_LOGI(TAG, "OUTPUT: type=%d, scale=%.6f, zp=%d", output->type, output->params.scale, output->params.zero_point);
...
void ModelHandler::debug_input_quantization_analysis(const uint8_t *input_data, size_t input_size,
  ...
  ESP_LOGI(TAG, "Input size: %zu bytes (%zu elements)", input_size,
           input_size / (input->type == kTfLiteFloat32 ? sizeof(float) : 1));
  ...
  if (input->type == kTfLiteUInt8) {
    ...
    for (size_t i = 0; i < float_count; i++) {
      ...
      max_val = std::max(max_val, float_data[i]);
      sum += float_data[i];
      if (float_data[i] == 0.0f)
        zero_count++;
    }

    float mean = sum / float_count;
    ESP_LOGI(TAG, "Input data stats: min=%.3f, max=%.3f, mean=%.3f", min_val, max_val, mean);
    ESP_LOGI(TAG, "Zero values: %d/%zu (%.1f%%)", zero_count, float_count, (zero_count * 100.0f) / float_count);
  }
...
void ModelHandler::debug_input_data_stats(const uint8_t *input_data, size_t input_size) const {
  ...
  ESP_LOGI(TAG, "Counts - zeros: %d, low (<50): %d, high (>200): %d", zero_count, low_count, high_count);
...
void ModelHandler::debug_quantized_input_details(TfLiteTensor *input, size_t input_size) const {
  ...
  ESP_LOGI(TAG, "Input quantization - scale: %.6f, zero_point: %d", input->params.scale, input->params.zero_point);
...
void ModelHandler::debug_output_tensor_details(TfLiteTensor *output) const {
  ...
  ESP_LOGI(TAG, "Output quantization - scale: %.6f, zero_point: %d", output->params.scale, output->params.zero_point);
...
void ModelHandler::debug_raw_outputs(TfLiteTensor *output) const {
  ...
  if (output->type == kTfLiteUInt8 || output->type == kTfLiteInt8) {
    ESP_LOGI(TAG, "Quantized raw outputs (%s):", output->type == kTfLiteUInt8 ? "UINT8" : "INT8");
...
void ModelHandler::debug_qat_model_output() const {
  ...
  for (int i = 0; i < 3 && i < output_size_; i++) {
    ESP_LOGI(TAG, "  %d: value=%d, score=%.6f", i, predictions[i].second, predictions[i].first);
  }
...
size_t ModelHandler::get_arena_peak_bytes() const { return interpreter_ ? interpreter_->arena_used_bytes() : 0; }

TfLiteTensor *ModelHandler::input_tensor() const { return interpreter_ ? interpreter_->input(0) : nullptr; }

TfLiteTensor *ModelHandler::output_tensor() const { return interpreter_ ? interpreter_->output(0) : nullptr; }

void ModelHandler::debug_model_architecture() const {
  if (!tflite_model_ || !interpreter_)
    return;
...
void ModelHandler::test_configuration(const ModelConfig &config, const std::vector<std::vector<uint8_t>> &zone_data,
  ...
  for (size_t zone_idx = 0; zone_idx < zone_data.size(); zone_idx++) {
    ...
    if (invoke_model(zone_buffer.data(), zone_buffer.size())) {
      ...
      ESP_LOGI(TAG, "  Zone %d: Value=%.1f, Confidence=%.6f", zone_idx + 1, output.value, output.confidence);
    } else {
...
void ModelHandler::debug_test_parameters(const std::vector<std::vector<uint8_t>> &zone_data) {
  ...
  ESP_LOGI(TAG, "Testing %d zones with %zu configurations", zone_data.size(), generate_debug_configs().size());
  ...
  for (int i = 0; i < display_count; i++) {
    ...
    for (size_t zone_idx = 0; zone_idx < result.zone_confidences.size(); zone_idx++) {
      ESP_LOGI(TAG, "   Zone %d: Value=%.1f, Confidence=%.6f", zone_idx + 1, result.zone_values[zone_idx],
               result.zone_confidences[zone_idx]);
    }
    ESP_LOGI(TAG, "   ---");
  }
