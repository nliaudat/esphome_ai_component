...
void AnalogReader::preprocess_image(const uint8_t *img, int w, int h, int cx, int cy, int radius,
                                    NeedleType needle_type, std::vector<uint8_t> &output) {
  // Safe Resize Logic
    ...
    if (vec.size() != required_size) {
      ...
      if (vec.capacity() < required_size) {
        ...
        if (largest_block < required_size) {
          ESP_LOGE(TAG, "CRITICAL: Not enough memory to resize %s. Need %u, Largest Free %u", name, required_size,
                   largest_block);
          return false;
        }
      }
  ...
  if (!safe_resize(output, "output"))
    return;

  // Copy data to working buffer
  memcpy(output.data(), img, w * h);

  if (!safe_resize(this->scratch_buffer_, "scratch1"))
    return;
  if (!safe_resize(this->scratch_buffer_2_, "scratch2"))
    return;
...
void AnalogReader::apply_tophat(uint8_t *img, int w, int h, int kernel_size, std::vector<uint8_t> &scratch,
                                std::vector<uint8_t> &scratch2, NeedleType needle_type) {
  if (scratch.size() != static_cast<size_t>(w * h))
    scratch.resize(w * h);
  if (scratch2.size() != static_cast<size_t>(w * h))
    scratch2.resize(w * h);  // Should be resized by caller but safety
...
AnalogReader::DetectionResult AnalogReader::detect_radial_profile(const uint8_t *img, int w, int h,
  ...
  if (start_r >= max_radius_scan) {
    ESP_LOGW(TAG, "Deadzone (%dpx radius) exceeds scan radius (%dpx); no pixels can be sampled", deadzone_r,
             max_radius_scan);
  ...
  if (fabs(denom) > 0.0001f) {
    ...
    // If denominator is positive (valley), offset might be weird, but we expect peak (negative 2nd derivative -> denom
    // negative) With y2 as max, y1 < y2 and y3 < y2 => y1+y3 < 2y2 => y1 - 2y2 + y3 < 0. Formula: p = (y_minus -
    // y_plus) / (2 * (y_minus - 2*y_center + y_plus))? Checking: Let y2=10, y1=5, y3=5. p = (5-5)/(2*(5-20+5)) = 0.
    // Correct. Let y2=10, y1=8, y3=5. p = (8-5)/(2*(8-20+5)) = 3 / (2 * -7) = -0.21. Shift towards y1. Correct.
    best_angle += offset;
  }
  ...
  if (range > 50)
    confidence = 0.8f + (range / 200.0f);
  else
    confidence = range / 100.0f;
  if (confidence > 1.0f)
    confidence = 1.0f;
...
AnalogReader::DetectionResult AnalogReader::detect_hough_transform(const uint8_t *img, int w, int h,
                                                                   const DialConfig &dial) {
  int cx = w / 2;
  int cy = h / 2;
  int radius = std::min(cx, cy) - 2;
    ...
    for (int x = 1; x < w - 1; x++) {
      ...
      if (magnitude > 30) {
        ...
        if (q == 0) {  // Horizontal: Check Left/Right
          if (x > 0)
            n1 = abs(-img[(y - 1) * w + (x - 2)] - 2 * img[y * w + (x - 2)] - img[(y + 1) * w + (x - 2)] +
                     img[(y - 1) * w + x] + 2 * img[y * w + x] +
                     img[(y + 1) * w + x]);  // Approximation? No using calculated mag map is better
...
AnalogReader::DetectionResult AnalogReader::detect_template_match(const uint8_t *img, int w, int h,
                                                                  const DialConfig &dial) {
  int cx = w / 2;
  int cy = h / 2;
  int radius = std::min(cx, cy) - 2;
    ...
    for (int r = start_r; r < end_r; r++) {
      ...
      if (px >= 0 && px < w && py >= 0 && py < h) {
        ...
        if (dial.needle_type == NEEDLE_TYPE_LIGHT) {
          if (val > 128)
            is_needle_pixel = true;  // Simple threshold for now, or relative to avg
          score += val;              // Accumulate intensity
        } else {
          // Dark needle
          if (val < 128)
            is_needle_pixel = true;
        ...
        if (dial.needle_type == NEEDLE_TYPE_DARK && val > 150)
          gap_count++;
        else if (dial.needle_type == NEEDLE_TYPE_LIGHT && val < 100)
          gap_count++;
        else
          gap_count = 0;
