...
void MeterReaderTFLite::setup() {
  ...
  ESP_LOGI(TAG, "Stored original camera dimensions: %dx%d, format: %s", original_camera_width_, original_camera_height_,
           original_pixel_format_.c_str());
  ...
  if (camera_window_configured_) {
    ...
    this->set_timeout(3000, [this]() {
      ...
      if (!this->set_camera_window(camera_window_offset_x_, camera_window_offset_y_, camera_window_width_,
                                   camera_window_height_)) {
  ...
  this->set_timeout(30000, [this]() {
    ...
    image_processor_ = std::make_unique<ImageProcessor>(
        ImageProcessorConfig{camera_width_, camera_height_, pixel_format_}, &model_handler_);
...
void MeterReaderTFLite::update() {
  ...
  if (flash_light_ && flash_light_enabled_ && !flash_scheduled_ && !pause_processing_.load()) {
    ...
    ESP_LOGI(TAG, "Flash scheduling: interval=%ums, pre_time=%ums, schedule_time=%ums", update_interval_ms,
             flash_pre_time_, schedule_time);
    ...
    if (schedule_time > 0 && schedule_time < update_interval_ms) {
      ...
      ESP_LOGI(TAG, "Flash scheduled: ON in %ums, OFF in %ums", schedule_time, disable_time);
      ...
      ESP_LOGW(TAG, "Invalid schedule time: %ums (interval: %ums)", schedule_time, update_interval_ms);
    }
  } else {
    ESP_LOGD(TAG, "Flash not scheduled - enabled:%s scheduled:%s paused:%s", flash_light_enabled_ ? "Y" : "N",
             flash_scheduled_ ? "Y" : "N", pause_processing_.load() ? "Y" : "N");
  }
...
void MeterReaderTFLite::process_full_image(std::shared_ptr<camera::CameraImage> frame) {
  ...
  for (auto &result : processed_zones) {
    ...
    if (process_model_result(result, &value, &confidence)) {
      ...
      ESP_LOGD(TAG, "Zone result - Value: %.1f, Confidence: %.2f", value, confidence);
    } else {
  ...
  if (processing_success && !readings.empty()) {
    ...
    ESP_LOGI(TAG, "Reading: %.1f -> %.1f (valid: %s, confidence: %.1f%%, threshold: %.1f%%)", final_reading,
             validated_reading, is_valid ? "yes" : "no", avg_confidence * 100.0f, confidence_threshold_ * 100.0f);

    if (inference_logs_) {
      // Publish to inference logs text sensor
      char inference_log[150];
      snprintf(inference_log, sizeof(inference_log),
               "Reading: %.1f -> %.1f (valid: %s, confidence: %.1f%%, threshold: %.1f%%)", final_reading,
               validated_reading, is_valid ? "yes" : "no", avg_confidence * 100.0f, confidence_threshold_ * 100.0f);
    ...
    if (avg_confidence >= confidence_threshold_ && is_valid) {
      ...
      ESP_LOGW(TAG, "Reading NOT published - %s", !is_valid ? "validation failed" : "confidence below threshold");
    }
...
bool MeterReaderTFLite::process_model_result(const ImageProcessor::ProcessResult &result, float *value,
                                             float *confidence) {
...
void MeterReaderTFLite::set_camera_image_format(int width, int height, const std::string &pixel_format) {
  ...
  ESP_LOGD(TAG, "Camera format set: %dx%d, %s (original: %dx%d, %s)", width, height, pixel_format.c_str(),
...
float MeterReaderTFLite::combine_readings(const std::vector<float> &readings) {
  ...
  for (size_t i = 0; i < readings.size(); i++) {
    ...
    if (digit == 10) {
      digit = 0;
      ESP_LOGD(TAG, "Zone %d: Raw=%.1f -> Rounded=10 -> Wrapped to 0", i + 1, readings[i]);
    } else {
      ESP_LOGD(TAG, "Zone %d: Raw=%.1f -> Rounded=%d", i + 1, readings[i], digit);
    }
...
void MeterReaderTFLite::set_model_config(const std::string &model_type) { model_type_ = model_type; }
...
void MeterReaderTFLite::print_debug_info() {
  ...
  ESP_LOGI(TAG, "  Tensor Arena Size (Requested): %zu bytes (%.1f KB)", tensor_arena_size_requested_,
           tensor_arena_size_requested_ / 1024.0f);
  ESP_LOGI(TAG, "  Tensor Arena Size (Actual): %zu bytes (%.1f KB)", tensor_arena_allocation_.actual_size,
           tensor_arena_allocation_.actual_size / 1024.0f);
  ...
  ESP_LOGI(TAG, "  TOTAL Memory Footprint: %zu bytes (%.1f KB)", total_memory, total_memory / 1024.0f);

  memory_manager_.report_memory_status(tensor_arena_size_requested_, tensor_arena_allocation_.actual_size, peak_usage,
                                       model_length_);
  ESP_LOGI(TAG, "----------------------------------");
}
// void MeterReaderTFLite::print_debug_info() {
...
bool MeterReaderTFLite::load_model() {
  ...
  if (!model_handler_.load_model(model_, model_length_, tensor_arena_allocation_.data.get(),
                                 tensor_arena_allocation_.actual_size, config)) {
    ESP_LOGE(TAG, "Failed to load model into interpreter");
    return false;
  }

  ESP_LOGI(TAG, "Model loaded successfully");
  ESP_LOGI(TAG, "Input dimensions: %dx%dx%d", model_handler_.get_input_width(), model_handler_.get_input_height(),
...
bool MeterReaderTFLite::allocate_tensor_arena() {
  ...
  ESP_LOGI(TAG, "Tensor arena allocated successfully: %zu bytes", tensor_arena_allocation_.actual_size);
  return true;
}
...
void MeterReaderTFLite::set_debug_image(const uint8_t *data, size_t size) {
  debug_image_ = std::make_shared<DebugCameraImage>(data, size, camera_width_, camera_height_);
  ESP_LOGI(TAG, "Debug image set: %zu bytes (%dx%d)", size, camera_width_, camera_height_);
}
  ...
  if (debug_image_) {
    ...
    if (!processed_zones.empty() && processed_zones.size() == debug_zones.size()) {
      ...
      for (size_t zone_idx = 0; zone_idx < processed_zones.size(); zone_idx++) {
        ...
        zone_data.push_back(std::vector<uint8_t>(zone_result.data->get(), zone_result.data->get() + zone_result.size));
      ...
      ESP_LOGE(TAG, "Zone processing failed. Expected %d zones, got %d", debug_zones.size(), processed_zones.size());
    }
  } else {
    ESP_LOGE(TAG, "No debug image set to process.");
  }
...
void MeterReaderTFLite::debug_test_with_pattern() {
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      if (channels > 1)
        float_pattern[pos + 1] = (y / float(height)) * 255.0f;  // G
      if (channels > 2)
        float_pattern[pos + 2] = 128.0f;  // B
    }
  }
...
bool MeterReaderTFLite::set_camera_window(int offset_x, int offset_y, int width, int height) {
  ESP_LOGI(TAG, "Setting camera window to: offset(%d,%d), size(%dx%d)", offset_x, offset_y, width, height);
  ...
  bool success = camera_window_control_.set_window_with_reset(
      camera_, camera_control::CameraWindowControl::WindowConfig{offset_x, offset_y, width, height, true});
...
bool MeterReaderTFLite::reset_camera_window() {
  ...
  if (success) {
    ...
    success = camera_window_control_.reset_to_full_frame_with_dimensions(
        camera_, original_camera_width_, original_camera_height_, camera_width_, camera_height_);
    ...
    if (success) {
      ...
      ESP_LOGI(TAG, "Camera window reset to original: %dx%d, format: %s", camera_width_, camera_height_,
               pixel_format_.c_str());
...
void MeterReaderTFLite::reinitialize_image_processor() {
  if (image_processor_) {
    image_processor_ = std::make_unique<ImageProcessor>(
        ImageProcessorConfig{camera_width_, camera_height_, pixel_format_}, &model_handler_);
    ESP_LOGI(TAG, "ImageProcessor reinitialized with dimensions: %dx%d, format: %s", camera_width_, camera_height_,
             pixel_format_.c_str());
  } else {
