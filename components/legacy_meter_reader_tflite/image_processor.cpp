...
CropZone adjust_zone_for_jpeg(const CropZone &zone, int max_width, int max_height) {
  ...
  ESP_LOGD(TAG, "Adjusted zone from [%d,%d,%d,%d] to [%d,%d,%d,%d] for JPEG", zone.x1, zone.y1, zone.x2, zone.y2,
 ...
 */
ImageProcessor::ImageProcessor(const ImageProcessorConfig &config, ModelHandler *model_handler)
    : config_(config), model_handler_(model_handler) {
  if (!config_.validate()) {
    ESP_LOGE(TAG, "Invalid image processor configuration");
    ESP_LOGE(TAG, "  Camera: %dx%d, Format: %s", config_.camera_width, config_.camera_height,
  ...
  ESP_LOGD(TAG, "  Model input: %dx%dx%d", model_handler_->get_input_width(), model_handler_->get_input_height(),
...
std::vector<ImageProcessor::ProcessResult> ImageProcessor::split_image_in_zone(
  ...
  if (zones.empty()) {
    ...
    for (size_t i = 0; i < zones.size(); i++) {
      ESP_LOGD(TAG, "Processing zone %d: [%d,%d,%d,%d]", i + 1, zones[i].x1, zones[i].y1, zones[i].x2, zones[i].y2);
  ...
  ESP_LOGD(TAG, "Image processing complete. Generated %d outputs in %lums", results.size(), processing_time);
  ESP_LOGD(TAG, "Success rate: %.1f%%, Avg time: %.1fms", stats_.get_success_rate(), stats_.get_avg_processing_time());
...
bool ImageProcessor::process_jpeg_zone_to_buffer(std::shared_ptr<camera::CameraImage> image, const CropZone &zone,
  ...
  if (!validate_buffer_size(required_size, output_buffer_size, "JPEG processing")) {
    ESP_LOGE(TAG, "Need %zu bytes for %s output, got %zu bytes", required_size,
             input_type == kTfLiteFloat32 ? "float32" : "uint8", output_buffer_size);
    return false;
  }

  // Step 1: Decode the full JPEG image to RGB888
  ESP_LOGD(TAG, "Decoding full JPEG image first for %s input", input_type == kTfLiteFloat32 ? "float32" : "uint8");
  ...
  ESP_LOGD(TAG, "Full JPEG decoded successfully to %dx%d RGB888", config_.camera_width, config_.camera_height);

#ifdef DEBUG_METER_READER_TFLITE
  debug_log_rgb888_image(full_image_buf, config_.camera_width, config_.camera_height, "jpeg_decoded_full");
#endif
  ...
  // Validate crop zone
  if (zone.x1 < 0 || zone.y1 < 0 || zone.x2 > config_.camera_width || zone.y2 > config_.camera_height) {
    ESP_LOGE(TAG, "Crop zone [%d,%d,%d,%d] out of bounds [0,0,%d,%d]", zone.x1, zone.y1, zone.x2, zone.y2,
...
#ifdef DEBUG_METER_READER_TFLITE
  debug_log_rgb888_image(cropped_buf, crop_width, crop_height, "jpeg_cropped");
#endif
  ...
  if (input_type == kTfLiteFloat32) {
    scale_success = scale_rgb888_to_float32(cropped_buf, crop_width, crop_height, output_buffer, model_width,
                                            model_height, model_channels, model_handler_->get_config().normalize);
  } else if (input_type == kTfLiteUInt8) {
    scale_success = scale_rgb888_to_uint8(cropped_buf, crop_width, crop_height, output_buffer, model_width,
                                          model_height, model_channels);
  }
  ...
  if (input_type == kTfLiteFloat32) {
    debug_analyze_float_zone(reinterpret_cast<float *>(output_buffer), model_width, model_height, model_channels,
                             "jpeg_final_output", model_handler_->get_config().normalize);
    // debug_output_float_preview(reinterpret_cast<float*>(output_buffer),
    // model_width, model_height, model_channels,
    // "jpeg_final_preview",
    // model_handler_->get_config().normalize);
  } else if (input_type == kTfLiteUInt8) {
    debug_analyze_processed_zone(output_buffer, model_width, model_height, model_channels, "jpeg_final_output");
  ...
  ESP_LOGD(TAG, "JPEG zone processed successfully: %dx%d -> %dx%d -> %dx%d", config_.camera_width,
           config_.camera_height, crop_width, crop_height, model_width, model_height);
...
bool ImageProcessor::validate_zone(const CropZone &zone) const {
  if (zone.x1 < 0 || zone.y1 < 0 || zone.x2 > config_.camera_width || zone.y2 > config_.camera_height) {
    ESP_LOGE(TAG, "Zone [%d,%d,%d,%d] out of bounds [0,0,%d,%d]", zone.x1, zone.y1, zone.x2, zone.y2,
             config_.camera_width, config_.camera_height);
    return false;
  }

  if (zone.x2 <= zone.x1 || zone.y2 <= zone.y1) {
    ESP_LOGE(TAG, "Invalid zone dimensions: width=%d, height=%d", zone.x2 - zone.x1, zone.y2 - zone.y1);
    return false;
  }
...
bool ImageProcessor::process_raw_zone_to_buffer(std::shared_ptr<camera::CameraImage> image, const CropZone &zone,
  ...
  ESP_LOGD(TAG, "Processing %s and input type %s", config_.pixel_format.c_str(), tflite_type_to_string(input_type));
  ...
  if (config_.pixel_format == "RGB888") {
    if (input_type == kTfLiteFloat32) {
      success = process_rgb888_crop_and_scale_to_float32(input_data, zone, crop_width, crop_height, output_buffer,
                                                         model_width, model_height, model_channels,
                                                         model_handler_->get_config().normalize);
    } else if (input_type == kTfLiteUInt8) {
      success = process_rgb888_crop_and_scale_to_uint8(input_data, zone, crop_width, crop_height, output_buffer,
                                                       model_width, model_height, model_channels);
    }
  } else if (config_.pixel_format == "RGB565") {
    if (input_type == kTfLiteFloat32) {
      success = process_rgb565_crop_and_scale_to_float32(input_data, zone, crop_width, crop_height, output_buffer,
                                                         model_width, model_height, model_channels,
                                                         model_handler_->get_config().normalize);
    } else if (input_type == kTfLiteUInt8) {
      success = process_rgb565_crop_and_scale_to_uint8(input_data, zone, crop_width, crop_height, output_buffer,
                                                       model_width, model_height, model_channels);
    }
  } else if (config_.pixel_format == "GRAYSCALE") {
    if (input_type == kTfLiteFloat32) {
      success = process_grayscale_crop_and_scale_to_float32(input_data, zone, crop_width, crop_height, output_buffer,
                                                            model_width, model_height, model_channels,
                                                            model_handler_->get_config().normalize);
    } else if (input_type == kTfLiteUInt8) {
      success = process_grayscale_crop_and_scale_to_uint8(input_data, zone, crop_width, crop_height, output_buffer,
                                                          model_width, model_height, model_channels);
    }
  } else {
  ...
  ESP_LOGD(TAG, "Raw zone processed successfully: %dx%d -> %dx%d -> %dx%d", crop_width, crop_height, model_width,
           model_height);

#ifdef DEBUG_METER_READER_TFLITE
  if (input_type == kTfLiteFloat32) {
    debug_log_float_image(reinterpret_cast<float *>(output_buffer), model_width * model_height * model_channels,
                          model_width, model_height, model_channels, "raw_processed_float");
  } else if (input_type == kTfLiteUInt8) {
    debug_log_image(output_buffer, required_size, model_width, model_height, model_channels, "raw_processed_uint8");
  }
#endif
...
bool ImageProcessor::scale_rgb888_to_float32(const uint8_t *input_data, int input_width, int input_height,
                                             uint8_t *output_buffer, int output_width, int output_height,
                                             int output_channels, bool normalize) {
  float *float_output = reinterpret_cast<float *>(output_buffer);
  ...
  for (int y = 0; y < output_height; y++) {
    int src_y = static_cast<int>(y * y_scale);
    if (src_y >= input_height)
      src_y = input_height - 1;

    for (int x = 0; x < output_width; x++) {
      int src_x = static_cast<int>(x * x_scale);
      if (src_x >= input_width)
        src_x = input_width - 1;
...
bool ImageProcessor::scale_rgb888_to_uint8(const uint8_t *input_data, int input_width, int input_height,
                                           uint8_t *output_buffer, int output_width, int output_height,
                                           int output_channels) {
  // Simple nearest-neighbor scaling
  float x_scale = static_cast<float>(input_width) / output_width;
...
#ifdef DEBUG_METER_READER_TFLITE
void ImageProcessor::debug_log_image_stats(const uint8_t *data, size_t size, const std::string &stage) {
  if (!data || size == 0)
    return;
  ...
  ESP_LOGD(TAG, "DEBUG %s: size=%zu, min=%u, max=%u, mean=%.1f, zeros=%d/%zu (%.1f%%)", stage.c_str(), size, min_val,
           max_val, mean, zero_count, size, (zero_count * 100.0f) / size);
...
void ImageProcessor::debug_log_float_stats(const float *data, size_t count, const std::string &stage) {
  if (!data || count == 0)
    return;
  ...
  ESP_LOGD(TAG, "DEBUG %s: count=%zu, min=%.3f, max=%.3f, mean=%.3f, zeros=%d/%zu (%.1f%%)", stage.c_str(), count,
           min_val, max_val, mean, zero_count, count, (zero_count * 100.0f) / count);
...
void ImageProcessor::debug_log_image(const uint8_t *data, size_t size, int width, int height, int channels,
  ...
  ESP_LOGD(TAG, "DEBUG %s: %dx%dx%d (%zu bytes)", stage.c_str(), width, height, channels, size);
...
void ImageProcessor::debug_log_float_image(const float *data, size_t count, int width, int height, int channels,
  ...
  ESP_LOGD(TAG, "DEBUG %s: %dx%dx%d (%zu floats)", stage.c_str(), width, height, channels, count);

  debug_log_float_stats(data, count, stage);
}

void ImageProcessor::debug_log_rgb888_image(const uint8_t *data, int width, int height, const std::string &stage) {
  if (!data || width <= 0 || height <= 0)
    return;

  size_t size = width * height * 3;
  ESP_LOGD(TAG, "DEBUG %s: %dx%d RGB (%zu bytes)", stage.c_str(), width, height, size);

  debug_log_image_stats(data, size, stage);

  // Log RGB values for first pixel
  if (size >= 3) {
    ESP_LOGD(TAG, "DEBUG %s first pixel: R=%u, G=%u, B=%u", stage.c_str(), data[0], data[1], data[2]);
  }
}
...
void ImageProcessor::debug_analyze_processed_zone(const uint8_t *data, int width, int height, int channels,
  ...
  ESP_LOGI(TAG, "ZONE_ANALYSIS:%s:%dx%dx%d:normalized=false", zone_name.c_str(), width, height, channels);
...
void ImageProcessor::debug_analyze_float_zone(const float *data, int width, int height, int channels,
                                              const std::string &zone_name, bool normalized) {
  if (!data || width <= 0 || height <= 0)
    return;

  const size_t total_pixels = width * height;
  ESP_LOGI(TAG, "ZONE_ANALYSIS:%s:%dx%dx%d:normalized=%s", zone_name.c_str(), width, height, channels,
  ...
  for (int y = 0; y < height; y++) {
    ...
    for (int x = 0; x < width; x++) {
      ...
      if (channels == 1) {
        ...
        brightness = static_cast<uint8_t>(0.299 * data[pos] + 0.587 * data[pos + 1] + 0.114 * data[pos + 2]);
      }
...
void ImageProcessor::debug_output_float_preview(const float *data, int width, int height, int channels,
  ...
  ESP_LOGI(TAG, "FLOAT_PREVIEW_START:%s:%dx%d:normalized=%s", zone_name.c_str(), width, height,
           normalized ? "true" : "false");
