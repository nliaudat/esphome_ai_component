...
void DrawingUtils::draw_pixel(uint8_t *buffer, int x, int y, int w, int h, int channels, uint16_t color) {
  ...
  if (channels == 2) {
    ...
    buffer[index + 1] =
        static_cast<uint8_t>(color >> 8);  // High byte (ESP32 is Little Endian usually, but check camera format)
...
void DrawingUtils::draw_rectangle(uint8_t *buffer, int x, int y, int rect_w, int rect_h, int img_w, int img_h,
...
void DrawingUtils::draw_filled_rectangle(uint8_t *buffer, int x, int y, int rect_w, int rect_h, int img_w, int img_h,
                                         int channels, uint16_t color) {
  for (int j = y; j < y + rect_h; ++j) {
    for (int i = x; i < x + rect_w; ++i) {
...
void DrawingUtils::draw_circle(uint8_t *buffer, int center_x, int center_y, int radius, int img_w, int img_h,
                               int channels, uint16_t color) {
  int x = 0;
  int y = radius;
  int d = 3 - 2 * radius;
...
void DrawingUtils::draw_filled_circle(uint8_t *buffer, int center_x, int center_y, int radius, int img_w, int img_h,
                                      int channels, uint16_t color) {
  int x = 0;
  int y = radius;
  int d = 3 - 2 * radius;
