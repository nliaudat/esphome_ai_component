...
ImageView Cropper::get_crop_view(const uint8_t *src, int src_w, int src_h, int channels, const CropZone &zone) {
...
bool Cropper::validate_zone(const CropZone &zone, int img_w, int img_h) {
  return (zone.x1 >= 0 && zone.y1 >= 0 && zone.x2 <= img_w && zone.y2 <= img_h && zone.x2 > zone.x1 &&
          zone.y2 > zone.y1);
}
