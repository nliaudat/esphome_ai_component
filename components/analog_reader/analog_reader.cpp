...
static bool decode_jpeg_to_buffer(const uint8_t *data, size_t len, jpeg_pixel_format_t output_format,
                                  uint8_t *output_buffer, size_t output_size, int *out_width, int *out_height) {
...
class PersistentDecodedImage : public esphome::camera::CameraImage {
 public:
  ...
  size_t get_data_length() override { return width_ * height_ * (format_ == PIXFORMAT_GRAYSCALE ? 1 : 3); }
...
AnalogReader::~AnalogReader() { this->persistent_buffer_.reset(); }
...
void AnalogReader::setup() {
  ...
  if (this->pixel_format_str_ == "JPEG") {
    ...
    ESP_LOGI(TAG, "Allocating persistent buffer: %u bytes (%dx%d)",
             static_cast<uint32_t>(this->persistent_buffer_size_), this->img_width_, this->img_height_);
    this->persistent_buffer_.reset(
        static_cast<uint8_t *>(heap_caps_malloc(this->persistent_buffer_size_, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT)));
...
class DecodedImage : public esphome::camera::CameraImage {
 public:
  ...
  ~DecodedImage() { ESP_LOGV("analog_reader", "Destroying DecodedImage (Buffer released)"); }
...
void AnalogReader::process_image_from_buffer(const uint8_t *data, size_t len) {
  ...
  ESP_LOGD(TAG, "Process Image From Buffer Start. Len: %u, Free Heap: %u", len,
  ...
  if (this->pixel_format_str_ == "JPEG") {
    if (this->persistent_buffer_) {
      ...
      jpeg_pixel_format_t out_fmt =
          (this->buffer_format_ == PIXFORMAT_GRAYSCALE) ? JPEG_PIXEL_FORMAT_GRAY : JPEG_PIXEL_FORMAT_RGB888;

      // Decode JPEG directly into pre-allocated persistent buffer (zero extra heap alloc)
      int dec_w = 0, dec_h = 0;
      bool ok = decode_jpeg_to_buffer(data, len, out_fmt, this->persistent_buffer_.get(), this->persistent_buffer_size_,
                                      &dec_w, &dec_h);
      if (ok) {
        out_w = dec_w;
        out_h = dec_h;
      ...
      if (ok) {
        if (out_w != this->img_width_ || out_h != this->img_height_) {
          ESP_LOGW(TAG,
                   "Decoded dimensions mismatch config: %dx%d vs %dx%d. Skipping analog processing to avoid conflict.",
                   out_w, out_h, this->img_width_, this->img_height_);
          return;
        }
        processing_image = std::make_shared<PersistentDecodedImage>(this->persistent_buffer_.get(), out_w, out_h,
                                                                    this->buffer_format_);
        is_decoded = true;
        processing_w = out_w;
        processing_h = out_h;
    ...
    if (!is_decoded) {
      ...
      int w, h;
      auto decoded_buf = esphome::esp32_camera_utils::ImageProcessor::decode_jpeg(data, len, &w, &h);

      if (decoded_buf) {
        processing_image =
            std::make_shared<DecodedImage>(std::move(decoded_buf), static_cast<size_t>(w), static_cast<size_t>(h));
        is_decoded = true;
        processing_w = w;
        processing_h = h;
  ...
  for (const auto &dial : this->dials_) {
    ...
    if (this->debug_)
      ESP_LOGD(TAG, "Processing Dial: %s (Algorithm: %s)", dial.id.c_str(), dial.algorithm.c_str());
    ...
    if (dial.process_channel != PROCESS_CHANNEL_GRAYSCALE && !dial.use_color) {
      ...
      int offset = 0;
      if (dial.process_channel == PROCESS_CHANNEL_RED)
        offset = 0;
      else if (dial.process_channel == PROCESS_CHANNEL_GREEN)
        offset = 1;
      else if (dial.process_channel == PROCESS_CHANNEL_BLUE)
        offset = 2;
      ...
      if (input_is_rgb) {
        ...
        ESP_LOGW(TAG, "Dial %s configured for channel %d but input is Grayscale", dial.id.c_str(),
                 static_cast<int>(dial.process_channel));
      }
    ...
    if (display_angle >= 360.0f)
      display_angle -= 360.0f;

    ESP_LOGD(TAG, "Dial %s: Angle=%.1f (North), Val=%.2f, Conf=%.2f, Time=%u us", dial.id.c_str(), display_angle, val,
             confidence, dur_algo);

    if (dial.angle_sensor)
      dial.angle_sensor->publish_state(display_angle);
    if (dial.confidence_sensor)
      dial.confidence_sensor->publish_state(confidence);
  ...
  if (this->stacked_digits_) {
    ...
    std::stable_sort(order.begin(), order.end(),
                     [&](int a, int b) { return this->readings_[a].dial->scale < this->readings_[b].dial->scale; });
    ...
    for (int p = 0; p < n; ++p) {
      ...
      if (this->debug_) {
        ESP_LOGD(TAG, "Stacked dial %s: raw=%.3f lower_frac=%.3f corrected=%.3f local_digit=%d phase=%.3f",
                 this->readings_[i].dial->id.c_str(), raw, lower_fraction, corrected, d, phase[i]);
      }
    }
    ...
    const long long rounded_steps = static_cast<long long>(steps + (steps >= 0.0f ? 0.5f : -0.5f));
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (this->debug_) {
        ESP_LOGD(TAG, "Stacked dial %s: final_digit=%d scale=%.7f", dial->id.c_str(), published_digit, dial->scale);
      }
    }
  } else {
    ...
    for (const auto &r : this->readings_) {
      ...
      if (r.dial->value_sensor)
        r.dial->value_sensor->publish_state(r.value);
  ...
  if (this->stacked_digits_ && !this->dials_.empty()) {
    ...
    for (const auto &d : this->dials_) {
      if (d.scale > 0.0f && d.scale < min_scale)
        min_scale = d.scale;
    }
    if (min_scale > 0.0f)
      precision = 1.0f / min_scale;
  }
...
AnalogReader::DetectionResult AnalogReader::find_needle_angle(const uint8_t *img, int w, int h,
  ...
  if (dial.algorithm == "auto") {
    ...
    if (result_radial.confidence > selected_result.confidence)
      selected_result = result_radial;
    if (result_hough.confidence > selected_result.confidence)
      selected_result = result_hough;
    if (result_template.confidence > selected_result.confidence)
      selected_result = result_template;
  } else {
    if (dial.algorithm == "legacy")
      selected_result = result_legacy;
    else if (dial.algorithm == "radial_profile")
      selected_result = result_radial;
...
float AnalogReader::angle_to_value(float image_angle, const DialConfig &dial) {
  ...
  if (!dial.calibration_mapping.empty()) {
    ...
    if (input_x <= dial.calibration_mapping.front().first)
      return dial.calibration_mapping.front().second;
    if (input_x >= dial.calibration_mapping.back().first)
...
void AnalogReader::debug_dial_image(const uint8_t *img, int w, int h, float detected_angle) {
  ...
  for (int r = max_r / 5; r < max_r; r++) {
    int draw_x = cx + static_cast<int>(cos(rad) * static_cast<float>(r) * static_cast<float>(grid_w) /
                                       static_cast<float>(max_r * 2));
    int draw_y = cy + static_cast<int>(sin(rad) * static_cast<float>(r) * static_cast<float>(grid_h) /
                                       static_cast<float>(max_r * 2));
  ...
  ESP_LOGD(TAG, "Legend: '%s#'%s=0-85 | '%s+'%s=86-170 | '%s.'%s=171-255 | %sX%s = Detected needle", COLOR_LOW,
           COLOR_RESET, COLOR_MID, COLOR_RESET, COLOR_HIGH, COLOR_RESET, COLOR_RED, COLOR_RESET);
  ...
  for (int y = 0; y < grid_h; y++) {
    ...
    for (int x = 0; x < grid_w; x++) {
      ...
      if (c == 'X') {
        ...
        if (val <= 85)
          colored_line += COLOR_LOW;
        else if (val <= 170)
          colored_line += COLOR_MID;
        else
          colored_line += COLOR_HIGH;
