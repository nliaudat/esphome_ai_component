...
from esphome.components import esp32, microphone, ota, psram, tflite_micro_helper
...
AUTO_LOAD = ["ring_buffer", "tflite_micro_helper"]
...
CONF_TFLITE_MODEL = "tflite_model"
CONF_TRAINED_LANGUAGES = "trained_languages"
CONF_VAD = "vad"
...
# Per-model entry: supports either a standalone model (model: ...) or a
# tflite_micro_helper reference (tflite_model: <TFLiteMicroHelper ID>).
# With tflite_model the user provides wake_word + trained_languages directly,
# and audio config is merged from the tflite_micro_helper entry.
TFLITE_MODEL_SCHEMA = cv.Schema(
    {
        cv.GenerateID(CONF_ID): cv.declare_id(WakeWordModel),
        cv.Required(CONF_TFLITE_MODEL): cv.use_id(
            tflite_micro_helper.TFLiteMicroHelper
        ),
        cv.Required(CONF_WAKE_WORD): cv.string,
        cv.Required(CONF_TRAINED_LANGUAGES): cv.ensure_list(cv.string),
...
MODEL_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_TFLITE_MODEL): cv.use_id(
            tflite_micro_helper.TFLiteMicroHelper
        ),
        cv.Optional(CONF_WAKE_WORD): cv.string,
        cv.Optional(CONF_TRAINED_LANGUAGES): cv.ensure_list(cv.string),
        cv.Optional(CONF_PROBABILITY_CUTOFF): cv.percentage,
        cv.Optional(CONF_SLIDING_WINDOW_SIZE): cv.positive_int,
        cv.Optional(CONF_INTERNAL, default=False): cv.boolean,
        cv.GenerateID(CONF_RAW_DATA_ID): cv.declare_id(cg.uint8),
...
def _validate_model_entry(value):
    ...
    has_model = CONF_MODEL in value
    has_tflite = CONF_TFLITE_MODEL in value

    if has_model and has_tflite:
        raise cv.Invalid(
            f"Cannot specify both '{CONF_MODEL}' and '{CONF_TFLITE_MODEL}' in the same model entry. "
            f"Use '{CONF_MODEL}' for standalone manifest-based models or '{CONF_TFLITE_MODEL}' "
            f"to reference a tflite_micro_helper model."
        )
    if not has_model and not has_tflite:
        raise cv.Invalid(
            f"Each model entry must have either '{CONF_MODEL}' (manifest-based) "
            f"or '{CONF_TFLITE_MODEL}' (reference to a tflite_micro_helper entry)."
        )
    if has_tflite:
        if CONF_WAKE_WORD not in value:
            raise cv.Invalid(
                f"When using '{CONF_TFLITE_MODEL}', you must specify '{CONF_WAKE_WORD}' "
                f"(the wake word name) because the tflite_micro_helper entry doesn't contain it."
            )
        if CONF_TRAINED_LANGUAGES not in value:
            raise cv.Invalid(
                f"When using '{CONF_TFLITE_MODEL}', you must specify '{CONF_TRAINED_LANGUAGES}' "
                f"(e.g., ['en']) because the tflite_micro_helper entry doesn't contain them."
            )
    ...
    cv.Schema(
        {
            ...
            cv.Required(CONF_MODELS): cv.ensure_list(_validate_model_entry),
...
TFLITE_DOMAIN = "tflite_micro_helper"


def _find_tflite_config(tflite_id):
    """Find the tflite_micro_helper config entry matching the given ID.
    Returns the entry config dict or None.
    During FINAL_VALIDATE_SCHEMA, CORE.config may not be populated yet,
    so returns None gracefully."""
    if CORE.config is None:
        return None
    tflite_configs = CORE.config.get(TFLITE_DOMAIN, [])
    for entry in tflite_configs:
        entry_id = entry.get(CONF_ID)
        if entry_id is not None and str(entry_id) == str(tflite_id):
            return entry
    return None


def _resolve_tflite_model_data(model_entry):
    """Resolve model data from a tflite_micro_helper reference.
    Returns (manifest_like_dict, model_bytes) where manifest_like_dict has
    the keys that to_code() expects.

    Looks up the tflite_micro_helper config from CORE.config to access
    the model source definition and re-read the model data.
    """
    tflite_id = model_entry[CONF_TFLITE_MODEL]
    tflite_cfg = _find_tflite_config(tflite_id)

    if tflite_cfg is None:
        raise cv.Invalid(
            f"tflite_micro_helper entry with id '{tflite_id}' not found. "
            f"Make sure it is defined in the tflite_micro_helper: section."
        )

    # Resolve model source using the tflite_micro_helper config dict
    model_path, model_data = tflite_micro_helper.resolve_model_source(tflite_cfg)

    # Parse audio config from the .txt file (if available)
    auto_config = tflite_micro_helper._parse_audio_config(model_path) or {}

    # Build a manifest-like dict with values from YAML or tflite_micro_helper's
    # auto-detected config. YAML overrides take precedence.
    manifest = {
        KEY_WAKE_WORD: model_entry[CONF_WAKE_WORD],
        KEY_TRAINED_LANGUAGES: model_entry[CONF_TRAINED_LANGUAGES],
        KEY_VERSION: 2,
        KEY_MICRO: {
            CONF_FEATURE_STEP_SIZE: auto_config.get("features_step_size", 20),
            CONF_TENSOR_ARENA_SIZE: auto_config.get("tensor_arena_size", 30000),
            CONF_PROBABILITY_CUTOFF: auto_config.get("probability_cutoff", 0.5),
            CONF_SLIDING_WINDOW_SIZE: auto_config.get("sliding_window_size", 10),
        },
    }

    return manifest, model_data
...
def _feature_step_size_validate(config):
    ...
    for model_parameters in config[CONF_MODELS]:
        if CONF_TFLITE_MODEL in model_parameters:
            ...
            tflite_cfg = _find_tflite_config(tflite_id)
            if tflite_cfg is None:
                # During FINAL_VALIDATE_SCHEMA: CORE.config may not be populated.
                # Skip step-size cross-validation for tflite_model refs.
                continue
            model_path, _ = tflite_micro_helper.resolve_model_source(tflite_cfg)
            auto_config = tflite_micro_helper._parse_audio_config(model_path) or {}
            model_step_size = auto_config.get("features_step_size", 20)
        else:
...
async def to_code(config):
    ...
    for i, model_parameters in enumerate(config[CONF_MODELS]):
        if CONF_TFLITE_MODEL in model_parameters:
            # Resolve model data from tflite_micro_helper reference
            manifest, data = _resolve_tflite_model_data(model_parameters)
        else:
            model_config = model_parameters.get(CONF_MODEL)
            data = []
        ...
        if manifest[KEY_WAKE_WORD] == "vad":
            ...
            # Only enable the first wake word by default. After first boot, the enable state
            # is saved/loaded to the flash
            default_enabled = i == 0
            ...
            for lang in manifest.get(KEY_TRAINED_LANGUAGES, []):
