...
class ModelHandler {
 public:
  bool load_model(const uint8_t *model_data, size_t model_size, uint8_t *tensor_arena, size_t tensor_arena_size,
  ...
  bool is_model_quantized() const { return interpreter_ && input_tensor() && input_tensor()->type == kTfLiteUInt8; }

  const uint8_t *get_quantized_output() const {
    return interpreter_ && output_tensor() ? output_tensor()->data.uint8 : nullptr;
  }

  float get_output_scale() const { return interpreter_ && output_tensor() ? output_tensor()->params.scale : 1.0f; }

  int get_output_zero_point() const { return interpreter_ && output_tensor() ? output_tensor()->params.zero_point : 0; }
