...
class DebugCameraImage : public camera::CameraImage {
 public:
  ...
  bool was_requested_by(camera::CameraRequester requester) const override { return false; }
...
void DebugCoordinator::print_info(const TFLiteCoordinator &tflite_coord, int cam_w, int cam_h, const std::string &fmt) {
  ...
  ESP_LOGI(TAG, "  Tensor Arena Size (Requested): %zu bytes (%.1f KB)", arena_req, arena_req / 1024.0f);

  size_t arena_act = tflite_coord.get_tensor_arena_size_actual();
  ESP_LOGI(TAG, "  Tensor Arena Size (Actual): %zu bytes (%.1f KB)", arena_act, arena_act / 1024.0f);
