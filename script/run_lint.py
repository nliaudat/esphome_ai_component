...
def fix_crlf_trailing_ws():
    ...
    count = 0
    for ext in ('*.h', '*.cpp', '*.tcc', '*.py', '*.c', '*.yaml', '*.yml', '*.md',
                '*.json', '*.cfg', '*.conf', '*.txt', '*.js', '*.html', '*.css'):
...
def main():
    ...
    parser.add_argument('--changed', action='store_true', help='Only check changed files')
    ...
    if tool_available("clang-format"):
        if args.fix:
            checks.append({
                ...
                "glob": ["components/**/*.h", "components/**/*.cpp", "components/**/*.tcc"],
                "batch_files": True,
            })
        else:
            checks.append({
    ...
    if args.fix:
        start = time.time()
        print(f"[CRLF/trailing WS fix] ", end="", flush=True)
    ...
    print()
    print(highlight("=" * 70))
    print(highlight(f"  Results: {passed}/{total} passed, {failed} failed, {skipped} skipped"))
    if failed > 0:
    ...
    if EXIT_CODE != 0 and not args.fix:
        print(highlight(f"  Tip: Run with --fix to auto-correct CRLF, trailing WS, ruff, clang-format"))
    print(highlight("=" * 70))
