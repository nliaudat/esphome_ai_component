...
def parse_zone_analysis(log_text):
    ...
    for line in log_text.split("\n"):
        ...
        zone_process_match = re.search(
            r"Processing zone (\d+): \[(\d+),(\d+),(\d+),(\d+)\]", line
        )
        ...
        zone_match = re.search(
            r"ZONE_ANALYSIS:([^:]+):(\d+)x(\d+)x(\d+):normalized=(\w+)", line
        )
        if zone_match:
        ...
        if pixel_match and current_zone:
            ...
            current_pixel = {"x": x, "y": y, "channels": {}}
        ...
        stats_match = re.search(
            r"Stats: min=([\d.]+), max=([\d.]+), mean=([\d.]+)", line
        )
        ...
        if current_zone and (
            "Zone processed successfully" in line
            or "process_zone duration:" in line
            or "JPEG zone processed successfully:" in line
            or re.search(r"Processing zone \d+:", line)  # New zone starting
            or re.search(r"ZONE_ANALYSIS:", line)  # New zone analysis starting
        ):
            current_zone = None
            current_pixel = None
...
def print_analysis(zones):
    ...
    for zone_data in zones.values():
        ...
        if zone_data["coordinates"]:
            ...
            print(
                f"  Coordinates: [{coords['x1']},{coords['y1']},{coords['x2']},{coords['y2']}] "
                f"(W:{coords['width']}, H:{coords['height']})"
            )

        if zone_data["stats"]:
            print(
                f"  Reported Stats: min={zone_data['stats']['min']:.6f}, max={zone_data['stats']['max']:.6f}, mean={zone_data['stats']['mean']:.6f}"
            )
        else:
...
def reconstruct_image(zones):
    ...
    for zone_unique_name, zone_data in zones.items():
        if zone_data["pixels"]:
            ...
            img_array = np.zeros(
                (zone_data["height"], zone_data["width"], zone_data["channels"]),
                dtype=np.uint8,
            )
            ...
            if zone_data["channels"] == 1:
                ...
                print(
                    f"Warning: Unsupported number of channels ({zone_data['channels']}) for {zone_unique_name}"
                )
                continue
...
for zone_unique_name, zone_data in zones.items():
    if "calculated_stats" in zone_data:
        ...
        print(
            f"{zone_unique_name}: min={stats['min']:.1f}, max={stats['max']:.1f}, mean={stats['mean']:.1f}, pixels={stats['count']}"
        )
