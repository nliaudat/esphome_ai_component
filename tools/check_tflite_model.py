...
def safe_analyze_model(model_path):
    ...
    try:
        if hasattr(tf.lite, "experimental") and hasattr(
...
def model_summary(interpreter):
    ...
    for op in ops_details:
        ...
        for tensor_idx in op["inputs"] + op["outputs"]:
            if tensor_idx < len(tensor_details):
                ...
                if tensor["name"] and any(
                    keyword in tensor["name"].lower()
                    for keyword in ["weight", "bias", "kernel"]
                ):
    ...
    summary_lines.append(
        f"{'Layer (type)':<25} {'Output Shape':<20} {'Param #':<15} {'Size (KB)':<10}"
    )
    summary_lines.append("-" * 80)
    ...
    for i, info in enumerate(layer_info):
        ...
        summary_lines.append(
            f"{layer_name:<25} {output_shape_str:<20} {params_str:<15} {memory_str:<10}"
        )
    ...
    for op in ops_details:
        ...
        for tensor_idx in op["inputs"]:
            if tensor_idx < len(tensor_details):
                ...
                summary_lines.append(
                    f"    Input {tensor_idx}: shape={shape_str}, dtype={tensor['dtype']}"
                )
        ...
        for tensor_idx in op["outputs"]:
            if tensor_idx < len(tensor_details):
                ...
                shape_str = str(tensor["shape"])
                summary_lines.append(
                    f"    Output {tensor_idx}: shape={shape_str}, dtype={tensor['dtype']}"
                )
...
def get_memory_analysis(interpreter):
    ...
    summary_lines.append(
        f"{'Tensor Name':<30} {'Shape':<20} {'Dtype':<10} {'Elements':<12} {'Memory (KB)':<12}"
    )
    summary_lines.append("-" * 80)

    for tensor_info in memory_breakdown[:20]:  # Show top 20
        shape_str = (
            str(tensor_info["shape"])[:18] + "..."
            if len(str(tensor_info["shape"])) > 18
            else str(tensor_info["shape"])
        )
        summary_lines.append(
...
def check_delegate_ops(interpreter):
    ...
    if delegate_ops:
        ...
        summary_lines.append(
            "   The model was likely exported with XNNPACK delegate enabled."
        )
        summary_lines.append(
            "   Re-export with: converter.target_spec.supported_ops = [tf.lite.OpsSet.TFLITE_BUILTINS]"
        )
    ...
    summary_lines.append(
        f"   Recommended MAX_OPERATORS: {recommended_max_ops} (total ops + 5 margin)"
    )
    summary_lines.append(
        f"   Set via build flag: -DMAX_OPERATORS={recommended_max_ops}"
    )
...
def get_peak_memory_analysis(interpreter):
        ...
        for tensor_idx, t in enumerate(tensor_details):
            if tensor_idx in tensor_first_use and tensor_idx in tensor_last_use:
                ...
                if tensor_idx not in all_outputs and tensor_idx not in model_inputs:
                    continue
                if (
                    tensor_first_use[tensor_idx]
                    <= op_idx
                    <= tensor_last_use[tensor_idx]
                ):
    ...
    peak_op_name = (
        ops_details[peak_op_idx]["op_name"] if peak_op_idx < len(ops_details) else "END"
    )
    ...
    summary_lines.append(
        f"Peak concurrent tensor memory (lifetime analysis): {peak_concurrent / 1024:.2f} KB at op {peak_op_idx} ({peak_op_name})"
    )
    summary_lines.append(
        f"Planner overhead: {SAFETY_FACTOR:.1f}x safety factor + {head_overhead}B head"
    )
    summary_lines.append(
        f"Estimated arena size: {arena_bytes_aligned / 1024:.2f} KB ({arena_bytes_aligned} bytes)"
    )
    summary_lines.append(
    ...
    for idx, t in enumerate(tensor_details):
        ...
        for dim in t["shape"]:
            if dim is not None:
                num_elements *= dim
        tensor_memory.append(
            (
                idx,
                dtype_size * num_elements / 1024,
                t["shape"],
                t["dtype"],
                t.get("name", ""),
            )
    ...
    for idx, mem_kb, shape, dtype, name in tensor_memory[:10]:
        summary_lines.append(
            f"  [{idx:3d}] {mem_kb:8.2f} KB  {str(shape):<25} {str(dtype):<20} {name[:50]}"
        )
    ...
    try:
        ...
        if evaluate and test_data:
            ...
            if eval_results:
                ...
                output_content.append(
                    f"Average Confidence: {eval_results['avg_confidence']:.4f} "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Run debug analysis with sample inference"
    )
    parser.add_argument(
        "--input_shape",
        type=str,
