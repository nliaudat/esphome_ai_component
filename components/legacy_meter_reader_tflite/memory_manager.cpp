...
MemoryManager::AllocationResult MemoryManager::allocate_tensor_arena(size_t requested_size) {
  ...
  // First try PSRAM
  uint8_t *arena_ptr = static_cast<uint8_t *>(heap_caps_malloc(requested_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));

  if (!arena_ptr) {
    ESP_LOGW(TAG, "PSRAM allocation failed, trying internal RAM");
    arena_ptr = static_cast<uint8_t *>(heap_caps_malloc(requested_size, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT));
  }
...
static void log_current_memory_usage(const char *context) {
  ...
  ESP_LOGD(TAG, "[%s] Free: Internal=%zuB, PSRAM=%zuB", context, free_internal, free_psram);
}
