...
def parse_model_txt_file(txt_path: Path) -> ModelConfig | None:
    ...
    logger.info(
        f"  Output processing: {config['output_processing']} (SOFTMAX={'yes' if has_softmax else 'no'})"
    )
    ...
    if re.search(r"shape=\[1 1 1 3\]", content):
        ...
        logger.info("  Quantization: QAT (learned 1x1 conv)")
    else:
        config["quantization_family"] = "tqt"
        logger.info("  Quantization: TQT")
...
def discover_models(models_dirs: list[Path] | None = None) -> dict[str, ModelConfig]:
    ...
    for models_dir in models_dirs:
        ...
        for txt_file in sorted(models_dir.glob("*.txt")):
            ...
            if config:
                models[name] = config
                logger.info(
                    f"Discovered model '{name}': {config.input_channels}ch {config.input_size} {config.input_type}"
                )
    if not models:
        logger.warning("No models discovered from any directory")
    return models
