...
class CropZoneHandler {
 public:
  ...
  globals::GlobalsComponent<std::string> *get_crop_zones_global() const { return crop_zones_global_; }
  ...
  void set_on_zones_changed_callback(std::function<void()> callback) { on_zones_changed_ = callback; }
