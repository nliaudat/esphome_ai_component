...
void Esp32CameraUtils::setup() {
  if (this->has_config_) {
    ESP_LOGI(TAG, "Setting up camera window: offset=%d,%d size=%dx%d", this->offset_x_, this->offset_y_, this->width_,
             this->height_);
    if (this->camera_) {
      ...
      bool success = this->window_control_.set_window_with_reset(
          this->camera_,
          CameraWindowControl::WindowConfig{this->offset_x_, this->offset_y_, this->width_, this->height_, true});
...
void Esp32CameraUtils::dump_config() {
  ...
  if (this->has_config_) {
    ESP_LOGCONFIG(TAG, "  Camera Window: offset=%d,%d size=%dx%d", this->offset_x_, this->offset_y_, this->width_,
                  this->height_);
  }
}
...
void Esp32CameraUtils::reinitialize_image_processor(const ImageProcessorConfig &config_template) {
  ...
  if (this->camera_width_ > 0 && this->camera_height_ > 0) {
    ...
    ESP_LOGI(TAG, "ImageProcessor initialized with dimensions: %dx%d, format: %s", this->camera_width_,
             this->camera_height_, this->pixel_format_.c_str());
...
bool Esp32CameraUtils::reset_window(int &width, int &height) {
  ...
  bool success = this->window_control_.reset_to_full_frame_with_dimensions(
      this->camera_, this->original_camera_width_, this->original_camera_height_, width, height);
...
bool Esp32CameraUtils::process_zone(std::shared_ptr<camera::CameraImage> frame, const CropZone &zone,
  ...
  if (this->has_config_) {
    ...
    if (local_zone.x2 <= local_zone.x1 || local_zone.y2 <= local_zone.y1) {
      ESP_LOGW(TAG,
               "Crop zone outside of current camera window (Global: %d,%d->%d,%d | Window Offset: %d,%d | Local: "
               "%d,%d->%d,%d)",
               zone.x1, zone.y1, zone.x2, zone.y2, this->offset_x_, this->offset_y_, local_zone.x1, local_zone.y1,
               local_zone.x2, local_zone.y2);
      return false;
    }
  }
